...
void Baud_Start(void) 
{
    ...
    Baud_CLKEN |= Baud_CLKEN_MASK;
	Baud_CLKSTBY |= Baud_CLKSTBY_MASK;
}
...
void Baud_Stop(void) 
{
    ...
    Baud_CLKEN &= (uint8)(~Baud_CLKEN_MASK);
	Baud_CLKSTBY &= (uint8)(~Baud_CLKSTBY_MASK);
}
...
{
    if ((Baud_CLKEN & Baud_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Baud_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Baud_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(Baud_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Baud_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Baud_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Baud_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Baud_GetSourceRegister();
    uint16 oldDivider = Baud_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Baud_CLKEN & Baud_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Baud_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Baud_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Baud_DIV_PTR);
}


/*******************************************************************************
* Function Name: Baud_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Baud_MOD_SRC |= modeBitMask & (uint8)Baud_MODE_MASK;
}


/*******************************************************************************
* Function Name: Baud_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Baud_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Baud_MODE_MASK));
}


/*******************************************************************************
* Function Name: Baud_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Baud_MOD_SRC & (uint8)(Baud_MODE_MASK);
}


/*******************************************************************************
* Function Name: Baud_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Baud_GetDividerRegister();
    uint8 oldSrc = Baud_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Baud_MOD_SRC |= CYCLK_SSS;
        Baud_MOD_SRC =
            (Baud_MOD_SRC & (uint8)(~Baud_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Baud_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Baud_MOD_SRC & Baud_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Baud_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Baud_PHASE = clkPhase & Baud_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Baud_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Baud_PHASE & Baud_PHASE_MASK;
}
