...
void BaudGen_Start(void) 
{
    ...
    BaudGen_CLKEN |= BaudGen_CLKEN_MASK;
	BaudGen_CLKSTBY |= BaudGen_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: BaudGen_Stop
********************************************************************************
*
* Summary:
...
void BaudGen_Stop(void) 
{
    ...
    BaudGen_CLKEN &= (uint8)(~BaudGen_CLKEN_MASK);
	BaudGen_CLKSTBY &= (uint8)(~BaudGen_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: BaudGen_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((BaudGen_CLKEN & BaudGen_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = BaudGen_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = BaudGen_CLKEN_MASK;
        ...
        oldDivider = CY_GET_REG16(BaudGen_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(BaudGen_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: BaudGen_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: BaudGen_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = BaudGen_GetSourceRegister();
    uint16 oldDivider = BaudGen_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = BaudGen_CLKEN & BaudGen_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((BaudGen_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: BaudGen_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(BaudGen_DIV_PTR);
}


/*******************************************************************************
* Function Name: BaudGen_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    BaudGen_MOD_SRC |= modeBitMask & (uint8)BaudGen_MODE_MASK;
}


/*******************************************************************************
* Function Name: BaudGen_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    BaudGen_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(BaudGen_MODE_MASK));
}


/*******************************************************************************
* Function Name: BaudGen_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return BaudGen_MOD_SRC & (uint8)(BaudGen_MODE_MASK);
}


/*******************************************************************************
* Function Name: BaudGen_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = BaudGen_GetDividerRegister();
    uint8 oldSrc = BaudGen_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        BaudGen_MOD_SRC |= CYCLK_SSS;
        BaudGen_MOD_SRC =
            (BaudGen_MOD_SRC & (uint8)(~BaudGen_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: BaudGen_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return BaudGen_MOD_SRC & BaudGen_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: BaudGen_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    BaudGen_PHASE = clkPhase & BaudGen_PHASE_MASK;
}


/*******************************************************************************
* Function Name: BaudGen_GetPhase
********************************************************************************
*
* Summary:
...
{
    return BaudGen_PHASE & BaudGen_PHASE_MASK;
}
