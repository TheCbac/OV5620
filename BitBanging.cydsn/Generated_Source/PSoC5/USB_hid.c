...
extern uint8 CYCODE USB_HIDREPORT_DESCRIPTORS[];
...
{
    uint8 status = USB_IDLE_TIMER_INDEFINITE;
    ...
    if(USB_hidIdleRate[interface] != 0u)
    {
        if(USB_hidIdleTimer[interface] > 0u)
        {
            ...
            status = USB_IDLE_TIMER_RUNNING;
        }
        else
        {
            USB_hidIdleTimer[interface] = USB_hidIdleRate[interface];
            status = USB_IDLE_TIMER_EXPIRED;
        }
    }
...
*
*******************************************************************************/
uint8 USB_DispatchHIDClassRqst() 
{
...
*
*******************************************************************************/
void USB_FindHidClassDecriptor() 
{
    T_USB_LUT *pTmp;
...
void USB_FindReportDescriptor() 
{
    ...
    pTmp += 3;
    pDescr = (volatile uint8 *)pTmp->p_list;
    USB_currentTD.count =  (((uint16)pDescr[1] << 8u) | pDescr[0u]);
    ...
    if(interfaceN < USB_MAX_INTERFACES_NUMBER)
    {
           ...
           (reportType <= USB_HID_GET_REPORT_FEATURE))
        {
            ...
            if(pTmp->c >= reportType)
            {
                ...
                USB_currentTD.pData = pTD->pData;   /* Bufer pointer */
                USB_currentTD.count = pTD->count;   /* Bufer Size */
