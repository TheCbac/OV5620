...
CY_ISR(USB_EP_0_ISR)
{
    ...
    if (bRegTemp & USB_MODE_ACKD)
    {
        modifyReg = 1u;
        if (bRegTemp & USB_MODE_SETUP_RCVD)
        {
            ...
            {
                USB_HandleSetup();
                if(USB_ep0Mode & USB_MODE_SETUP_RCVD)
                {
        ...
        else if (bRegTemp & USB_MODE_IN_RCVD)
        {
            USB_HandleIN();
        }
        else if (bRegTemp & USB_MODE_OUT_RCVD)
        {
            USB_HandleOUT();
        }
        else
        ...
        if(modifyReg != 0u)
        {
            ...
            if((bRegTemp & USB_MODE_SETUP_RCVD) == 0)  /* Check if SETUP bit is not set, otherwise exit */
            {
                /* Update the count register */
                bRegTemp = USB_ep0Toggle | USB_ep0Count;
                CY_SET_REG8(USB_EP0_CNT_PTR, bRegTemp);
                if(bRegTemp == CY_GET_REG8(USB_EP0_CNT_PTR))   /* continue if writing was succesful */
                {
                    do
                    {
...
void USB_HandleSetup(void) 
{
    ...
    if(requestHandled & USB_MODE_SETUP_RCVD)
    {
...
*     current packet.
*  USB_lastPacketSize - remebers the USBFS_ep0Count value for the
*     next packet.
*  USB_transferByteCount - suma of the previos bytes transferred
...
void USB_LoadEP0(void) 
{
    ...
    USB_ep0Count = 0u;
    while ((USB_currentTD.count > 0u) && (USB_ep0Count < 8u))
    {
        USB_EP0_DR0_PTR[USB_ep0Count] = *USB_currentTD.pData++;
        USB_ep0Count++;
        USB_currentTD.count--;
    }
    /* Support zero-length packet*/
    if( (USB_ep0Count > 0u) || (USB_lastPacketSize == 8u) )
    {
    ...
    USB_lastPacketSize = USB_ep0Count;
}
...
{
    uint16 xferCount;
    if(USB_currentTD.count == 0)
    {
        USB_InitZeroLengthControlTransfer();
    }
    else
    {
...
void USB_ControlWriteDataStage(void) 
{
    uint8 *pReg = (uint8 *)USB_EP0_DR0_PTR;

    USB_ep0Count = (CY_GET_REG8(USB_EP0_CNT_PTR ) & 0x0Fu) - 2u;

    USB_transferByteCount += USB_ep0Count;

    while ((USB_currentTD.count > 0u) && (USB_ep0Count > 0u))
    {
        *USB_currentTD.pData++ = CY_GET_REG8(pReg++);
        USB_ep0Count--;
        USB_currentTD.count--;
    }
    /* Update the data toggle */
