...
#define USB_MAX_REPORTID_NUMBER   0
...
typedef struct _USB_EpCtlBlock
{
    uint8  attrib;
    uint8  apiEpState;
    uint8  hwEpState;
...
typedef struct _USB_EpSettingsBlock
{
    uint8  interface;
    uint8  altSetting;
    uint8  addr;
...
typedef struct _USB_XferStatusBlock
{
    uint8  status;
    uint16 length;
} T_USB_XFER_STATUS_BLOCK;

typedef struct _USB_Td
{
    uint16  count;
...
typedef struct _USB_BackupStruct
{
    uint8 enableState;
    uint8 mode;
} USB_BACKUP_STRUCT;
...
void   USB_Resume(void) ;
    ...
    cystatus USB_CyBtldrCommWrite(uint8 *pData, uint16 size, uint16 *count, uint8 timeOut) CYSMALL
                                                        ;
...
#define USB_EnableOutISOCEP(e)         USB_EnableOutEP(e)

#define USB_SetVector                  CyIntSetVector
#define USB_SetPriority                CyIntSetPriority
#define USB_EnableInt(a, b)            CyIntEnable(a)
...
#if(CY_PSOC5A)
    #define USB_PM_AVAIL_CR_PTR    (  (reg8 *) CYREG_PM_AVAIL_CR6)
    #define USB_PM_AVAIL_CR_REG    (* (reg8 *) CYREG_PM_AVAIL_CR6)
#else
...
#if(CY_PSOC5A)
    #define USB_PM_AVAIL_CR    USB_PM_AVAIL_CR_PTR
#endif /* End CY_PSOC5A */
...
#define USB_CR1_BUS_ACTIVITY           (0x01u << USB_CR1_BUS_ACTIVITY_SHIFT)
#if(!CY_PSOC5A)
...
#define USB_EPX_CNT0_MASK              (0x07u)
...
#define USB_CR0_ENABLE                 (0x80u)
    ...
    #define USB_BUS_RST_COUNT              (0x03u)
    #define USB_USBIO_CR1_IOMODE           (0x80u)
    #define USB_USBIO_CR1_DRIVE_MODE       (0x40u)
    #define USB_USBIO_CR1_DPI              (0x20u)
    #define USB_USBIO_CR1_DMI              (0x10u)
    #define USB_USBIO_CR1_P2PUEN           (0x08u)
#else
...
#endif /* End USB_EP_MM == USB__EP_DMAAUTO */
#define USB_ARB_INT_MASK              ((USB_DMA1_REMOVE ^ 1) | \
                                                    (USB_DMA2_REMOVE ^ 1) << 1 | \
                                                    (USB_DMA3_REMOVE ^ 1) << 2 | \
                                                    (USB_DMA4_REMOVE ^ 1) << 3 | \
                                                    (USB_DMA5_REMOVE ^ 1) << 4 | \
