...
#define USB_CyBtLdrStarttimer(X, T)         {USB_universalTime = T * 10; X = 0u;}
#define USB_CyBtLdrChecktimer(X)            ((X++ < USB_universalTime) ? 1u : 0u)
...
uint16 USB_universalTime = 0u;
uint8 USB_started = 0u;
                                                            ...
                                                            
{
    ...
    if(USB_started)
    {
        ...
        while(!USB_GetConfiguration() && USB_CyBtLdrChecktimer(time))
        {
            CyDelay(1); /* 1ms delay */
        }
